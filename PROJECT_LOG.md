...
### Invoices (faktura) + PDF, fully compliant (2026-06)

A complete Swedish faktura with PDF, working on **both PC and phone**:

- **Schema** (v3, migrated): `company` (seller profile), `payment_method`
  (Swish/Bankgiro/IBAN — label + value), `invoice` (UNBROKEN sequential
  `invoice_number`, encrypted buyer snapshot, frozen seller + payment-method JSON),
  `invoice_line` (articles: desc/qty/unit price/rate), `rut_recipient` (household
  RUT split — each person's name + encrypted personnummer + share).
- **Operations**: `create_invoice` computes lines + moms per rate, snapshots
  buyer/seller/methods, splits RUT across recipients (Luhn-validated), and issues
  the underlying PENDING income (so the existing pending→paid booking + reports just
  work). `get_invoice` / `list_invoices`, company + payment-method CRUD.
- **PDF**: `backend/invoices/pdf.py` via **fpdf2** (pure-pip; verified under Pyodide,
  so it renders on the phone too). Seller/buyer blocks, line table, moms summary,
  per-recipient RUT boxes with skattereduktion, "Att betala" after RUT, payment methods.
- **API + UI**: `/company`, `/payment-methods`, `/invoices` (+ `/pdf`); a "Fakturor"
  section (list + PDF) with a line-item editor and a RUT-recipient editor; company +
  payment methods in Inställningar.
- Verified end-to-end in a real browser on **both** the desktop server and the phone
  WASM bundle (invoice created in the UI → PDF generated by fpdf2 in-page, no server).
